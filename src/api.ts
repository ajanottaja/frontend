...
type ActiveTarget =
  paths["/targets/active"]["get"]["responses"]["200"]["schema"];
...
type ActiveTargetResponse = ActiveTargetResponseOK;

interface ActiveTargetArgs {
  auth0: Auth0ContextInterface<User>;
}

export const activeTarget = async ({
  auth0,
}: ActiveTargetArgs): Promise<ActiveTargetResponse> => {
  const url = "https://api.ajanottaja.app/targets";
