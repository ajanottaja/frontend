...
const Timer = ({ beginning, title, startInterval, stopInterval }: Timer) => {
  ...
  return <div display="grid" grid="cols-1 gap-y-8 <lg:gap-y-4" justify="self-center">
    ...
    <span className={`${beginning ? "text-gray-300" : "text-gray-700"} font-mono`}
      text="6xl <lg:4xl">
    ...
    {beginning ?
      <IconButton icon={faStop} ariaLabel="Start interval" onClick={async () => {
        await stopInterval();
      }}>
        Stop interval
      </IconButton> :
      <IconButton icon={faPlay} ariaLabel="Start interval" onClick={async () => {
        await startInterval();
      }}>
        Start interval
      </IconButton>}
